...
# Language detection
def detect_language(email_text):
    try:
        language = detect(email_text)
        return language  # Returns 'en' for English, 'de' for German, etc.
...
def translate_to_english(email_text):
    try:
        translated = translator.translate(email_text, src='de', dest='en')
        return translated.text
    except Exception as e:
        print(f"Error translating email: {e}")
        return email_text  # Return original text if translation fails

# Add language detection and translation to preprocessing
def preprocess_email(email_text):
    # Detect language
    language = detect_language(email_text)

    # Translate if the email is in German
    if language == 'de':
        email_text = translate_to_english(email_text)

    # Continue with existing preprocessing (e.g., cleaning, vectorizing)
...
test_email = "Herzlichen Glückwunsch! Sie haben gewonnen."
print(preprocess_email(test_email))  # Should return the email translated to English

input_your_mail =test_email
input_data_features = feature_extraction.transform([input_your_mail])
prediction = model.predict(input_data_features)
if prediction[0] == 1:
    print("Ham Mail")
